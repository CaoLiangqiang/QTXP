...
尺度按真实米制（层高 3.0 m、进深 11 m、檐高 28.2 m），共 2902 个物体、29 种材质。

**建筑设计语言来自宣传图的视觉复核**：连续圆角挑檐、逐层 Y 形/拱形竖向构架、
转角玻璃包角、通透架空层、香槟色金属收边。早期版本只是方盒 + 平直阳台，
与宣传图差距明显；当前模型用多段 smoothstep 宽度曲线构造拱架，圆角实体挑板用
5 段 Bevel，底层改成后部核心筒 + 前玻璃 + 纤细立柱。

**建模内容**：楼体分层、外挑楼板线条、单元分界竖向壁柱、女儿墙、屋顶机房与电梯井、
南立面逐户窗（含室内凹深暗盒 + 两道竖棂 + 一道横档）、**逐户阳台**（挑板 + 地面 +
玻璃栏板 + 金属扶手 + 侧向隔板）、**空调外机**、非住宅基座、北立面窗；
场地外有内部铺装、楼间草坪与步道、南北两条市政道路（含车道虚线）、约 70 棵行道树与
组团树、10 栋邻楼体量——空场地是"假"的最大来源，粗糙的文脉也远胜一块灰板。

**逐户差异**是让渲染"不像模型"的最大杠杆，比贴图分辨率重要得多：真实住宅楼不存在
两扇一样的窗。每户按房号做 FNV-1a 哈希得到确定性伪随机数（同一户每次渲染长得一样），
据此分配窗面状态——约 22% 白窗帘、12% 暖窗帘、8% 灰窗帘、12% 哑光玻璃、12% 亮面玻璃，
其余反射玻璃；约 12% 室内为暖色微发光（像有人在家），空调外机的有无与左右位置也逐户不同。

**材质配色来自售楼处宣传物料的反推**，不是凭感觉调的。物料是手机翻拍的印刷页
（本地 `宣传图/`，因开发商版权不入库）。初版因模型无法读取图片，先用
`.build/blender/extract_palette.py` 把每张图降采样成 6×9 空间网格与 40×40 细网格直接读 RGB；
切换到具备视觉能力的模型后，又对五张图做了逐张视觉复核，补全了几何设计语言。

| 素材 | 视觉内容 | 用于建模/取色的关键信息 |
| --- | --- | --- |
| IMG_1885 | 南立面正视效果图 | 连续圆角挑檐、逐层 Y 形构架、通透底层、草坪前景 |
| IMG_1886 | 转角近景仰视 | 拱形竖向框架、香槟金收边、弧形玻璃转角、深阳台 |
| IMG_1887 | 顶层转角特写 | 包角玻璃、圆角实体檐口、蓝灰玻璃 `#4d545b` `#485057` `#56626f` |
| IMG_1888 | 架空层与内庭 | 挑高架空、竖向格栅、弧形玻璃盒、暖石材 `#d3bfa5` `#bfb09c` |
| IMG_1889 | 全项目鸟瞰总平 | 围合式组团、道路与密集绿化；景观 `#487858` `#386848`、铺装 `#ae9971` |

这套数据纠正了两处我原先凭感觉调错的材质：**玻璃**原用深藏青 `#0f2e45`，实测立面玻璃
是中间调蓝灰 `#4f5a63`，亮得多；**草坪**原用黄绿 `#4a7a2c`，实际是偏蓝的深绿 `#3f6f50`。

> 局限：素材是印刷物料的手机翻拍，带印刷色偏与拍摄环境光，所以这些值反映的是
> **色相关系**（暖立面 ↔ 冷玻璃 ↔ 深绿景观），不是绝对色值。仍未使用任何照片作为贴图
> 纹理，所有材质为程序化生成（多尺度噪声凹凸 + 基色微扰）。拿到电子版立面图可进一步校正。

| 产物 | 说明 |
| --- | --- |
| `renders/10_黄昏鸟瞰.jpg` | 冬至 14:45 真实低角度暖光（高度角 21.4°、西南向），侧逆光 |
| `renders/11_近景人视.jpg` | 1.9 m 真人视高贴近 11幢 南立面，34 mm + f/7 景深，看阳台细节 |
...
- **效果图画廊**：左侧「📷 Blender 效果图与日照动画」按钮或 <kbd>G</kbd> 打开，
  8 张效果图 + 2 段视频，点图放大。图片走相对路径 `renders/`，
  所以 HTML 需与 `renders/` 同层；单独挪走 HTML 时画廊会明确提示缺文件而不是留空白
